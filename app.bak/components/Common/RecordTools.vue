<template>
  ...
  >
    <v-btn-toggle v-model="stateStore.recordShowContent" mandatory density="compact">
      <v-btn disabled text="查看" :value="'markdown'" />
      <v-btn disabled text="编辑" :value="'edit'" />
    </v-btn-toggle>
    <div class="ml-auto">
      <v-btn
        disabled
    ...
    <div v-if="recordStore.record.id">
      <v-btn disabled variant="plain" text="更新" class="font-weight-bold" @click="update()" />
    </div>
    <div v-else>
      <v-btn disabled variant="plain" text="保存" class="font-weight-bold" @click="insert()" />
    </div>
  </v-card>
</template>
