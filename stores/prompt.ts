export const usePromptStore = defineStore(
  ...
  () => {
    ...
    async function getPrompts() {
      try {
        type Response = { status: string; data: [] }
        const response = (await databaseRouter.prompt('list')) as Response
        if (response.status === 'OK') {
          const promptList = ['case', 'story', 'test', 'act', 'rate']
          promptList.forEach((item) => {
            prompts.value.system[item].user = response.data.filter((i: any) => i.type === item)
          })
          console.log(prompts.value.system)
          stateStore.appInfo = '列表成功'
        } else {
          stateStore.appInfo = '列表失败: ' + response.data
        }
      } catch (error) {
    ...
    function setCurrentSystemPrompt(systemPromptType: SystemPromptType, value: string) {
      prompts.value.system[systemPromptType].current = value
    }

    function getCurrentSystemPrompt(systemPromptType: SystemPromptType): string {
      return (
        prompts.value.system[systemPromptType].current ||
        prompts.value.system[systemPromptType].default
      )
    }
    function getSystemPrompt(systemPromptType: SystemPromptType): Messages {
      ...
      return [
        { role: 'system', content: getCurrentSystemPrompt(systemPromptType) },
        {
          role: 'user',
          content: content,
        },
    ...
    return { prompts, getSystemPrompt, setCurrentSystemPrompt, getPrompts }
  },
  {
    persist: true,
  }
