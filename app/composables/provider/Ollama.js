export const useProviderOllama = () => {
  const FREE_API_KEY_NAME = ''
  const USER_API_KEY_NAME = 'USER_OLLAMA_API_KEY'
  const API_BASE = 'http://localhost:11434/api'
  const CHAT_COMPLETIONS = '/chat'
    ...
    while (true) {
      ...
      for (const line of lines) {
        ...
        try {
          ...
          //   : ''
          modelResponseStream.content += message.message.content ? message.message.content : ''
  ...
  async function getChatResponse(modelUsage, messages) {
    ...
    if (chatModel.thinking) {
      payload.body.think = stateStore.isModelThinking
    }
    ...
    if (response.status !== 200) {
      ...
      stateStore.appInfos.push(errorFromModel.error)
      return
    }
