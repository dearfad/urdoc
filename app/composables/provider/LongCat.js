...
export const useProviderLongCat = () => {
  const FREE_API_KEY_NAME = 'LONGCAT_API_KEY'
  const USER_API_KEY_NAME = 'USER_LONGCAT_API_KEY'
  const API_BASE = 'https://api.longcat.chat/openai/v1'
    ...
    while (true) {
      ...
      for (const line of lines) {
        if (line.startsWith('data:')) {
          if (line.slice(5).trim() === '[DONE]') break // 结束标志
          data = line.slice(5).trimStart() // 去掉冒号后所有前导空格
        } else {
          continue
        }
        try {
