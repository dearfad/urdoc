<template>
  <v-container class="flex-fill bg-blue-lighten-4 overflow-auto">
    ...
    <v-list class="chatMsgContainer h-100 font-weight-bold" v-if="isChatting">
      <v-list-item v-for="(message, index) in useChatMessages.slice(1)" :key="index"
        :title="message.role + ':'">
        {{ message.content }}
      </v-list-item>
      <v-list-item class="chatMsgBottom"></v-list-item>
    </v-list>
  </v-container>
</template>
